...
class CaptioningTransformer(nn.Module):
    ...
    def forward(self, features, captions):
        ...
        decoder_features = self.visual_projection(features)  # (N, WD)
        decoder_features = torch.unsqueeze(decoder_features, 1)  # (N, S=1, WD)
        captions_embedding = self.embedding(captions) # (N, T, WD)
        captions_embedding = self.positional_encoding(captions_embedding)

        tgt_mask = torch.ones(size=(T, T))
        tgt_mask = torch.tril(tgt_mask)
        out = self.transformer(captions_embedding, decoder_features, tgt_mask)   # (N, T, WD)
