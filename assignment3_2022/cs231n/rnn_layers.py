...
def rnn_forward(x, h0, Wx, Wh, b):
    ...
    for t in range(T):
        xt = x[:, t, :]
        ht, cache_t = rnn_step_forward(xt, prevh, Wx, Wh, b)
        # print(ht.shape)
        h[:, t, :] = ht
...
def lstm_step_forward(x, prev_h, prev_c, Wx, Wh, b):
    ...
    cache = (next_h, next_c, i, f, o, g, prev_h, prev_c, x, Wx, Wh)
...
def lstm_step_backward(dnext_h, dnext_c, cache):
    ...
    #############################################################################
    # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    next_h, next_c, i, f, o, g, prev_h, prev_c, x, Wx, Wh = cache
    N, H = dnext_h.shape
...
def lstm_forward(x, h0, Wx, Wh, b):
    ...
    cache = []
    prevh = h0
    prevc = np.zeros(shape=(N, H))

    for t in range(T):
        xt = x[:, t, :]
        # ht, cache_t = rnn_step_forward(xt, prevh, Wx, Wh, b)
        ht, ct, cache_t = lstm_step_forward(xt, prevh, prevc, Wx, Wh, b)
        # print(ht.shape)
        h[:, t, :] = ht
        cache.append(cache_t)
        prevh = ht
